...
\item
  Manipuler des tableaux de données avec \texttt{tidyr} (\protect\hyperlink{ref-R-tidyr}{Wickham, 2021a}) pour les mettre dans un format permettant les analyses statistiques et les représentations graphiques.
\item
  Faire des graphiques exploratoires avec \texttt{ggplot2} (\protect\hyperlink{ref-R-ggplot2}{Wickham, Chang, et al., 2021}) pour visualiser des données.
\item
  Filtrer des lignes, sélectionner des colonnes, trier, créer de nouvelles variables et calculer des résumés des données avec les fonctions \texttt{filter()}, \texttt{select()}, \texttt{arrange()}, \texttt{mutate()}, \texttt{summarise()} et \texttt{group\_by()} du package \texttt{dplyr} (\protect\hyperlink{ref-R-dplyr}{Wickham, François, et al., 2021}).
\item
...
\item
  Téléchargez les fichiers de données disponibles dans ce document (chapitres ``Packages et données'' de chaque section principale : \ref{packages}, \ref{packages2} et \ref{packages3}) et placez-les dans le dossier du TP correspondant.
\item
  Lancez RStudio.
\item
...
Les packages dont vous aurez besoin pour cette séance, et que vous devez donc charger en mémoire, sont les packages du \texttt{tidyverse} (\protect\hyperlink{ref-R-tidyverse}{Wickham, 2021b}), qui permettent de manipuler facilement des tableaux de données et de réaliser des graphiques, le package \texttt{readr} (\protect\hyperlink{ref-R-readr}{Wickham \& Hester, 2021}), pour importer facilement des fichiers \texttt{.csv} au format \texttt{tibble}, le package \texttt{readxl} (\protect\hyperlink{ref-R-readxl}{Wickham \& Bryan, 2019}), pour importer facilement des fichiers Excel au format \texttt{tibble}, le package \texttt{skimr} (\protect\hyperlink{ref-R-skimr}{Waring et al., 2021}), qui permet de calculer des résumés de données très informatifs, et le package \texttt{car} (\protect\hyperlink{ref-R-car}{Fox et al., 2021}), qui permet d'effectuer le test de comparaison des variances de Levene :
...
Les données se trouvent dans \href{data/Testosterone.csv}{le fichier \texttt{Testosterone.csv}}. Importez ces données dans un objet nommé \texttt{Testo} et examinez le tableau obtenu.
...
\begin{Highlighting}[]
\NormalTok{Testo\_long }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Traitement, }\AttributeTok{y =}\NormalTok{ DO, }\AttributeTok{group =}\NormalTok{ ID, }\AttributeTok{color =}\NormalTok{ ID)) }\SpecialCharTok{+}
...
\FunctionTok{t.test}\NormalTok{(Testo\_large}\SpecialCharTok{$}\NormalTok{Diff, }\AttributeTok{mu =} \DecValTok{0}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{wilcox.test}\NormalTok{(Testo\_large}\SpecialCharTok{$}\NormalTok{Diff, }\AttributeTok{mu =} \DecValTok{0}\NormalTok{, }\AttributeTok{conf.int =} \ConstantTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Les données de cette étude sont stockées dans \href{data/HornedLizards.csv}{le fichier \texttt{HornedLizards.csv}}. Importez ces données dans un objet nommé \texttt{Lizard} et examinez le tableau obtenu.
...
\hypertarget{packages2}{%
\subsection{Packages et données}\label{packages2}}
...
Les packages dont vous aurez besoin pour cette séance, et que vous devez donc charger en mémoire, sont les mêmes que pour la section précédente : les packages du \texttt{tidyverse} (\protect\hyperlink{ref-tidyverse2019}{Wickham et al., 2019}), qui permettent de manipuler facilement des tableaux de données et de réaliser des graphiques, le package \texttt{readr} (\protect\hyperlink{ref-R-readr}{Wickham \& Hester, 2021}), pour importer facilement des fichiers \texttt{.csv} au format \texttt{tibble}, le package \texttt{readxl} (\protect\hyperlink{ref-R-readxl}{Wickham \& Bryan, 2019}), pour importer facilement des fichiers Excel au format \texttt{tibble}, le package \texttt{skimr} (\protect\hyperlink{ref-R-skimr}{Waring et al., 2021}), qui permet de calculer des résumés de données très informatifs, et le package \texttt{car} (\protect\hyperlink{ref-R-car}{Fox et al., 2021}), qui permet d'effectuer le test de comparaison de variances de Levene. Nous utiliserons aussi le package \texttt{broom} (\protect\hyperlink{ref-R-broom}{Robinson et al., 2021}), qui fait partie du \texttt{tidyverse} mais qu'il faut charger explicitement. La fonction \texttt{tidy()} de ce package nous permettra de ``ranger'' correctement les résultats de tests dans un \texttt{tibble} :
...
Enfin, nous aurons besoin du package \texttt{DescTools} (\protect\hyperlink{ref-R-DescTools}{Signorell, 2021}) afin de réaliser un test spécifique de comparaisons multiples. N'oubliez pas de l'installer si nécessaire, avant de le charger en mémoire :
...
Les données brutes de cette étude sont fournies dans le fichier \href{data/Light.csv}{\texttt{Light.csv}}. Importez ces données dans RStudio et examinez les données brutes grâce à la fonction \texttt{View()}.
...
Nous avons ici la confirmation que le design expérimental n'est pas équilibré, puisque le groupe \texttt{control} compte un individu de plus. Nous pouvons ensuite utiliser la fonction \texttt{skim} du package \texttt{skimr} pour obtenir un résumé des données :
...
Il semble que le groupe \texttt{eyes} se comporte un peu différemment des autres groupes. En effet, pour les groupes \texttt{control} et \texttt{knee}, les valeurs observées sont très proches :
...
\begin{center}\includegraphics[width=0.9\linewidth]{figure/unnamed-chunk-78-1} \end{center}
...
\begin{center}\includegraphics[width=0.9\linewidth]{figure/unnamed-chunk-84-1} \end{center}
...
\begin{center}\includegraphics[width=0.9\linewidth]{figure/unnamed-chunk-91-1} \end{center}
...
\hypertarget{packages3}{%
\subsection{Packages et données}\label{packages3}}
...
\begin{center}\includegraphics[width=0.9\linewidth]{figure/unnamed-chunk-100-1} \end{center}
...
\begin{center}\includegraphics[width=0.9\linewidth]{figure/unnamed-chunk-105-1} \end{center}
...
\begin{center}\includegraphics[width=0.9\linewidth]{figure/unnamed-chunk-108-1} \end{center}
...
Les données de Wiseman \& Lamont (\protect\hyperlink{ref-wiseman1996}{1996}) sont disponibles dans le fichier \href{data/ropetrick.csv}{\texttt{ropetrick.csv}}. Importez ces données et analysez-les en respectant les consignes de l'exercice précédent.
...
Les données de cette expérience sont disponibles dans le fichier \href{data/plantbiomass.csv}{\texttt{plantbiomass.csv}}.
...
\begin{center}\includegraphics[width=0.9\linewidth]{figure/unnamed-chunk-119-1} \end{center}
...
\begin{center}\includegraphics[width=0.9\linewidth]{figure/unnamed-chunk-124-1} \end{center}
...
\begin{center}\includegraphics[width=0.9\linewidth]{figure/unnamed-chunk-126-1} \end{center}
...
\FunctionTok{library}\NormalTok{(tidyverse)}
\FunctionTok{library}\NormalTok{(skimr)}
\NormalTok{loups }\OtherTok{\textless{}{-}} \FunctionTok{read\_csv}\NormalTok{(}\StringTok{"data/loups.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{skim}\NormalTok{(loups)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{loups }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ inbreedCoef, }\AttributeTok{y =}\NormalTok{ nPups)) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=0.9\linewidth]{figure/unnamed-chunk-131-1} \end{center}
...
\begin{Highlighting}[]
\NormalTok{loups }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{pull}\NormalTok{(inbreedCoef) }\SpecialCharTok{\%\textgreater{}\%} 
...
\begin{Highlighting}[]
\NormalTok{loups }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{pull}\NormalTok{(nPups) }\SpecialCharTok{\%\textgreater{}\%} 
\NormalTok{  shapiro.test}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{cor.test}\NormalTok{(loups}\SpecialCharTok{$}\NormalTok{inbreedCoef, loups}\SpecialCharTok{$}\NormalTok{nPups)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{rope }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{count}\NormalTok{(impressiveness)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{rope }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ years, }\AttributeTok{y =}\NormalTok{ impressiveness)) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{()}

\CommentTok{\# Test de Normalité de la première variable}
\NormalTok{rope }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{pull}\NormalTok{(years) }\SpecialCharTok{\%\textgreater{}\%} 
\NormalTok{  shapiro.test}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{rope }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{pull}\NormalTok{(impressiveness) }\SpecialCharTok{\%\textgreater{}\%} 
\NormalTok{  shapiro.test}
\end{Highlighting}
\end{Shaded}
...
\begin{center}\includegraphics[width=0.9\linewidth]{figure/unnamed-chunk-136-1} \end{center}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{cor.test}\NormalTok{(rope}\SpecialCharTok{$}\NormalTok{years, rope}\SpecialCharTok{$}\NormalTok{impressiveness, }
         \AttributeTok{method =} \StringTok{"spearman"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{datasaurus-et-anscombe-1}{%
\subsubsection{Datasaurus et Anscombe}\label{datasaurus-et-anscombe-1}}
...
\begin{center}\includegraphics[width=0.9\linewidth]{figure/unnamed-chunk-138-1} \end{center}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{hockey }\OtherTok{\textless{}{-}} \FunctionTok{read\_csv}\NormalTok{(}\StringTok{"data/hockey.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{skim}\NormalTok{(hockey)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{hockey }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ FaceWidthHeightRatio, }\AttributeTok{y =}\NormalTok{ PenaltyMinutes)) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{reg2 }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(PenaltyMinutes }\SpecialCharTok{\textasciitilde{}}\NormalTok{ FaceWidthHeightRatio, }\AttributeTok{data =}\NormalTok{ hockey)}
...
\begin{Highlighting}[]
\FunctionTok{par}\NormalTok{(}\AttributeTok{mfrow =} \FunctionTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{,}\DecValTok{2}\NormalTok{))}
\FunctionTok{plot}\NormalTok{(reg2)}
\FunctionTok{par}\NormalTok{(}\AttributeTok{mfrow =} \FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=0.9\linewidth]{figure/unnamed-chunk-144-1} \end{center}
...
\begin{center}\includegraphics[width=0.9\linewidth]{figure/unnamed-chunk-145-1} \end{center}
...
\begin{Highlighting}[]
\NormalTok{hockey }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{filter}\NormalTok{(FaceWidthHeightRatio}\SpecialCharTok{\textless{}}\DecValTok{2}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ FaceWidthHeightRatio, }\AttributeTok{y =}\NormalTok{ PenaltyMinutes)) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+}
...
\begin{center}\includegraphics[width=0.9\linewidth]{figure/unnamed-chunk-146-1} \end{center}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{reg3 }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(PenaltyMinutes }\SpecialCharTok{\textasciitilde{}}\NormalTok{ FaceWidthHeightRatio, }
           \AttributeTok{data =}\NormalTok{ hockey }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{filter}\NormalTok{(FaceWidthHeightRatio}\SpecialCharTok{\textless{}}\DecValTok{2}\NormalTok{))}
...
\leavevmode\vadjust pre{\hypertarget{ref-Campbell1998}{}}%
\textsc{Campbell SS}, \& \textsc{Murphy PJ}. (1998). Extraocular circadian phototransduction in humans. \emph{Science}, \textbf{279}(5349), 396--399. \url{https://doi.org/10.1126/science.279.5349.396}

\leavevmode\vadjust pre{\hypertarget{ref-carre2008}{}}%
\textsc{Carré JM}, \& \textsc{McCormick CM}. (2008). In your face: Facial metrics predict aggressive behaviour in the laboratory and in varsity and professional hockey players. \emph{Proceedings of the Royal Society B: Biological Sciences}, \textbf{275}(1651), 2651--2656. \url{https://doi.org/10.1098/rspb.2008.0873}

\leavevmode\vadjust pre{\hypertarget{ref-R-car}{}}%
\textsc{Fox J}, \textsc{Weisberg S}, \& \textsc{Price B}. (2021). \emph{Car: Companion to applied regression}. Retrieved from \url{https://CRAN.R-project.org/package=car}

\leavevmode\vadjust pre{\hypertarget{ref-Fuller2003}{}}%
\textsc{Fuller A}, \textsc{Kamerman PR}, \textsc{Maloney SK}, \textsc{Mitchell G}, \& \textsc{Mitchell D}. (2003). Variability in brain and arterial blood temperatures in free-ranging ostriches in their natural habitat. \emph{Journal of Experimental Biology}, \textbf{206}(7), 1171--1181. \url{https://doi.org/10.1242/jeb.00230}

\leavevmode\vadjust pre{\hypertarget{ref-Hasselquist1999}{}}%
\textsc{Hasselquist D}, \textsc{Marsh JA}, \textsc{Sherman PW}, \& \textsc{Wingfield JC}. (1999). Is avian humoral immunocompetence suppressed by testosterone? \emph{Behavioral Ecology and Sociobiology}, \textbf{45}(3), 167--175. \url{https://doi.org/10.1007/s002650050550}

\leavevmode\vadjust pre{\hypertarget{ref-liberg2005}{}}%
\textsc{Liberg O}, \textsc{Andrén H}, \textsc{Pedersen H-C}, \textsc{Sand H}, \textsc{Sejberg}, \textsc{Wabakken P}, \ldots{} \textsc{Bensch S}. (2005). Severe inbreeding depression in a wild wolf {Canis} lupus population. \emph{Biology Letters}, \textbf{1}(1), 17--20. \url{https://doi.org/10.1098/rsbl.2004.0266}

\leavevmode\vadjust pre{\hypertarget{ref-Molofsky2005}{}}%
\textsc{Molofsky J}, \& \textsc{Ferdy J-B}. (2005). Extinction dynamics in experimental metapopulations. \emph{Proceedings of the National Academy of Sciences}, \textbf{102}(10), 3726--3731. \url{https://doi.org/10.1073/pnas.0404576102}

\leavevmode\vadjust pre{\hypertarget{ref-muller2011}{}}%
\textsc{Müller MS}, \textsc{Porter ET}, \textsc{Grace JK}, \textsc{Awkerman JA}, \textsc{Birchler KT}, \textsc{Gunderson AR}, \ldots{} \textsc{Anderson DJ}. (2011). Maltreated nestlings exhibit correlated maltreatment as adults: {Evidence} of a {``cycle of violence''} in {Nazca Boobies} ( {\emph{Sula}}{ \emph{Granti}} ). \emph{The Auk}, \textbf{128}(4), 615--619. \url{https://doi.org/10.1525/auk.2011.11008}

\leavevmode\vadjust pre{\hypertarget{ref-R-broom}{}}%
\textsc{Robinson D}, \textsc{Hayes A}, \& \textsc{Couch S}. (2021). \emph{Broom: Convert statistical objects into tidy tibbles}. Retrieved from \url{https://CRAN.R-project.org/package=broom}
...
\leavevmode\vadjust pre{\hypertarget{ref-tilman2006}{}}%
\textsc{Tilman D}, \textsc{Reich PB}, \& \textsc{Knops JMH}. (2006). Biodiversity and ecosystem stability in a decade-long grassland experiment. \emph{Nature}, \textbf{441}(7093), 629. \url{https://doi.org/10.1038/nature04742}

\leavevmode\vadjust pre{\hypertarget{ref-R-skimr}{}}%
\textsc{Waring E}, \textsc{Quinn M}, \textsc{McNamara A}, \textsc{Arino de la Rubia E}, \textsc{Zhu H}, \& \textsc{Ellis S}. (2021). \emph{Skimr: Compact and flexible summaries of data}. Retrieved from \url{https://CRAN.R-project.org/package=skimr}

\leavevmode\vadjust pre{\hypertarget{ref-whitlock2015}{}}%
\textsc{Whitlock M}, \& \textsc{Schluter D}. (2015). \emph{The analysis of biological data} (Second edition). Greenwood Village, Colorado: {Roberts and Company Publishers}.
...
\textsc{Wickham H}, \textsc{Averick M}, \textsc{Bryan J}, \textsc{Chang W}, \textsc{McGowan LD}, \textsc{François R}, \ldots{} \textsc{Yutani H}. (2019). Welcome to the {tidyverse}. \emph{Journal of Open Source Software}, \textbf{4}(43), 1686. \url{https://doi.org/10.21105/joss.01686}

\leavevmode\vadjust pre{\hypertarget{ref-R-readxl}{}}%
\textsc{Wickham H}, \& \textsc{Bryan J}. (2019). \emph{Readxl: Read excel files}. Retrieved from \url{https://CRAN.R-project.org/package=readxl}

\leavevmode\vadjust pre{\hypertarget{ref-R-ggplot2}{}}%
\textsc{Wickham H}, \textsc{Chang W}, \textsc{Henry L}, \textsc{Pedersen TL}, \textsc{Takahashi K}, \textsc{Wilke C}, \ldots{} \textsc{Dunnington D}. (2021). \emph{ggplot2: Create elegant data visualisations using the grammar of graphics}. Retrieved from \url{https://CRAN.R-project.org/package=ggplot2}

\leavevmode\vadjust pre{\hypertarget{ref-R-dplyr}{}}%
\textsc{Wickham H}, \textsc{François R}, \textsc{Henry L}, \& \textsc{Müller K}. (2021). \emph{Dplyr: A grammar of data manipulation}. Retrieved from \url{https://CRAN.R-project.org/package=dplyr}

\leavevmode\vadjust pre{\hypertarget{ref-R-readr}{}}%
\textsc{Wickham H}, \& \textsc{Hester J}. (2021). \emph{Readr: Read rectangular text data}. Retrieved from \url{https://CRAN.R-project.org/package=readr}

\leavevmode\vadjust pre{\hypertarget{ref-wiseman1996}{}}%
\textsc{Wiseman R}, \& \textsc{Lamont P}. (1996). Unravelling the {Indian} rope-trick. \emph{Nature}, \textbf{383}(6597), 212. \url{https://doi.org/10.1038/383212a0}

\leavevmode\vadjust pre{\hypertarget{ref-Wright2002}{}}%
\textsc{Wright KP}, \& \textsc{Czeisler CA}. (2002). Absence of circadian phase resetting in response to bright light behind the knees. \emph{Science}, \textbf{297}(5581), 571--571. \url{https://doi.org/10.1126/science.1071697}

\leavevmode\vadjust pre{\hypertarget{ref-Young2004}{}}%
\textsc{Young KV}, \textsc{Brodie ED}, \& \textsc{Brodie ED}. (2004). How the horned lizard got its horns. \emph{Science}, \textbf{304}(5667), 65--65. \url{https://doi.org/10.1126/science.1094790}
