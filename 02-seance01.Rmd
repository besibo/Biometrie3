...
1. on utilise le symbole `~` pour indiquer une formule. On cherche à regarder l'effet du `Traitement` sur la `DO` qui traduit l'immuno-compétence. Le `~` soit se lire "en fonction de".
2. Avec la synatxe utilisant les formules, on doit spécifier l'argument `data = Testo\_long` pour indiquer à RStudio que les variables `DO` et `Traitement` sont des colonnes de ce tableau.
