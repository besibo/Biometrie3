...
knitr::write_bib(c(
  .packages(), 'bookdown', 'knitr', 'rmarkdown', 'tidyverse', 'ggplot2', 'gridExtra', 'skimr', 'bindrcpp', 'tidyr', 'dplyr', 'readr', "car", "readxl", "broom", "DescTools"), 'packages.bib')
```
...
list.of.packages <- c("readxl", "nycflights13", "gapminder", "knitr", "tidyverse", "gridExtra", "devtools", "extrafont", "kableExtra", "skimr", "bindrcpp", "car", "DescTools", "bookdown")
...
- [`Autruches.csv`](https://besibo.github.io/Biometrie3/data/Autruches.csv)
- [`HommesFemmes.xls`](https://besibo.github.io/Biometrie3/data/HommesFemmes.xls)
- [`HornedLizards.csv`](https://besibo.github.io/Biometrie3/data/HornedLizards.csv)
- [`Temperature.csv`](https://besibo.github.io/Biometrie3/data/Temperature.csv)
- [`Temperature2.csv`](https://besibo.github.io/Biometrie3/data/Temperature2.csv)
- [`Testosterone.csv`](https://besibo.github.io/Biometrie3/data/Testosterone.csv)
...
Notez ici que la forme de cet histogramme est très proche de celle présenté plus tôt par la fonction `skim()`. Cet histogramme nous apprend qu'en dehors d'un "trou" autour de la température 36.75 ºC, la distribution des données est proche d'une courbe en cloche. Il y a fort à parier qu'un test de normalité concluerait à la normalité des données de cet échantillon.
...
Les données se trouvent dans  [le fichier `Testosterone.csv`](https://besibo.github.io/Biometrie3/data/Testosterone.csv). Importez ces données dans un objet nommé `Testo` et examinez le tableau obtenu.
...
Les données de cette étude sont stockées dans [le fichier `HornedLizards.csv`](https://besibo.github.io/Biometrie3/data/HornedLizards.csv). Importez ces données dans un objet nommé `Lizard` et examinez le tableau obtenu.
...
- [`Light.csv`](https://besibo.github.io/Biometrie3/data/Light.csv)
- [`Insectes.csv`](https://besibo.github.io/Biometrie3/data/Insectes.csv)
...
Les données brutes de cette étude sont fournies dans le fichier [`Light.csv`](https://besibo.github.io/Biometrie3/data/Light.csv). Importez ces données dans RStudio et examinez les données brutes grâce à la fonction `View()`.
...
- [`birds.csv`](https://besibo.github.io/Biometrie3/data/birds.csv)
- [`loups.csv`](https://besibo.github.io/Biometrie3/data/loups.csv)
- [`ropetrick.csv`](https://besibo.github.io/Biometrie3/data/ropetrick.csv)
- [`plantbiomass.csv`](https://besibo.github.io/Biometrie3/data/plantbiomass.csv)
- [`hockey.csv`](https://besibo.github.io/Biometrie3/data/hockey.csv)
...
Les données de @wiseman1996 sont disponibles dans le fichier [`ropetrick.csv`](https://besibo.github.io/Biometrie3/data/ropetrick.csv). Importez ces données et analysez-les en respectant les consignes de l'exercice précédent.
...
Les données de cette expérience sont disponibles dans le fichier [`plantbiomass.csv`](https://besibo.github.io/Biometrie3/data/plantbiomass.csv).
...
datasaurus_dozen %>% 
    ...
    summarize(
      ...
      correl_x_y  = cor(x, y),
      pente = coef(lm(y~x))[2],
      ordonnee_origine = pente = coef(lm(y~x))[1]
    )
```

Examinez attentivement les nombreux résultats produits par cette commande. dans la console, ainsi que les 4 graphiques obtenus. Vous devriez remarquer que pour ces 13 jeux de données, 2 variables numériques `x` et `y` sont mises en relation. Pour tous ces jeux de données, on observe que la moyenne des `x` est la même, la moyenne de tous les `y` est la même, les écarts-types des `x` sont identiques, les écarts-types des `y` aussi, et la corrélation entre `x` et `y` est également extrêmement proche pour tes les jeux de données.


, `x3` et `y3`, `x4` et `y4` sont tous les mêmes. Enfin, les ordonnées à l'origine et les pentes de ces 4 droites de régression linéaires sont identiques. Pourtant, seule la première régression linéaire est valide. Pourquoi ?
